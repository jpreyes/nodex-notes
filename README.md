...
- **Correo:** en Configuración → Correo agrega tu cuenta (Gmail, Outlook, iCloud u otra con IMAP) con una **contraseña de aplicación**: una contraseña especial que se crea en tu cuenta (requiere la verificación en dos pasos) y sirve solo para esta app. Cada 15 minutos la app lee los correos nuevos de la última semana, de la bandeja de entrada y de enviados, sin marcarlos como leídos. La IA resume cada uno y lleva a Tareas y Agenda los compromisos (los tuyos y los que otros te hicieron, con `@Nombre`) y las fechas, con Deshacer. Si un correo parece cumplir algo pendiente (por ejemplo, llegan los planos que Juan prometió), la vista **Correos** te pregunta si marcarlo hecho. Los boletines y avisos automáticos se ignoran. Preguntar también conoce tus correos recientes. La contraseña queda en `config.toml` y los correos en `correos.json`, ambos solo en tu equipo; para entenderlos, su texto se envía al modelo de IA configurado.
